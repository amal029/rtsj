...
bound of \texttt{delay}. Figure~\ref{fig:graph1} shows the accuracy of
our results by comparing differences between programmer specified upper
bounds (Table~\ref{fig:benchlist}) and the actual bounds obtained using
our techniques (Table~\ref{fig:comparison}). 
...
\begin{tikzpicture}[scale=0.9]
	\begin{axis}[
			ybar,xtick=data,
			symbolic x
			coords={HRTCS,Printer/CD1,Printer/CD2,AECS/CD2},
			height=5cm,
			width=9.5cm,
			minor y tick num=1,
			yminorgrids=true,
...
%      xlabel=Benchmark,
			...
			\addplot coordinates
			{(HRTCS,1)(Printer/CD1,1)(Printer/CD2,1)(AECS/CD2,1.94)};
			\addplot coordinates {(Printer/CD2, 1)(AECS/CD2, 1.94)};
	\end{axis}
\end{tikzpicture}
\begin{tikzpicture}[scale=0.9]
	\begin{axis}[
...
%      nodes near coords
			...
			\addplot coordinates {(Motor, 2.60)};
			\addplot coordinates {(Motor, 2.63)};
			\addplot coordinates {(Motor, 4.39)};
			\addplot coordinates {(Motor, 2.92)};
			\addplot coordinates {(Motor, 2.69)};
...
Figure~\ref{fig:graph2}. As one can see from the Figure, the upper
bounds for delay statements in AECS/CD2 have been decreased by 11.81\%
which is higher than all those in Motor which ranges from 7.54 to
8.05\%. It was as expected since amount of computation required
in control flow is mostly less demanding than data such that breaking
...
depends on how designers write their program as well as typs of the
...
\begin{tikzpicture}[scale=0.9]
	\begin{axis}[
			...
			\addplot coordinates {(Motor, 8.00) (AECS/CD2, 11.82)};
