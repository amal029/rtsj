...
fixed real-time period, the period of the logical clock is
...
\hj{(1) real-time wait does not affect the model of logical time in
  ...
  that can be determined statically by program analysis, assuming that
  real-time analyzable platforms are available,} (2) the waiting model
relies on the use of relative instead of absolute real-time, i.e., a
wait in selected time units is counted from the beginning of the wait
