...
The previous results show that for HRTCS and Printer that the compiler
was able to find \textit{d} without relaxing upper bound of
\texttt{delay}. Figure~\ref{fig:graph1} shows the accuracy of our
results by comparing differences between programmer specified upper
bounds (Table~\ref{fig:benchlist}) and the actual bounds obtained using
our techniques (Table~\ref{fig:comparison}) for the other examples
requiring relaxation. AECS/CD1 delay is not included in
Figure~\ref{fig:graph2}, because although it requires relaxation, the
delay specification (Table~\ref{fig:benchlist}) does not have an upper
bound.
...
%      xlabel=Benchmark,
			...
			\addplot + [postaction={pattern=north west
                          lines}] coordinates 
...
%      nodes near coords
			...
			\addplot + [postaction={pattern=north east lines}] coordinates
                        {(Motor, 2.60)};
			\addplot + [postaction={pattern=north west lines}] coordinates
                        {(Motor, 2.63)};
			\addplot + [postaction={pattern=dots}] coordinates
                        {(Motor, 4.39)};
			\addplot + [postaction={pattern=horizontal lines}] coordinates
                        {(Motor, 2.92)};
			\addplot + [postaction={pattern=crosshatch}] coordinates
                        {(Motor, 2.69)};
...
Figure~\ref{fig:graph2}.  The upper bounds for delay statements in
...
\begin{tikzpicture}[scale=0.9]
	\begin{axis}[
			...
			\addplot + [fill=white,postaction={pattern=north west lines}] coordinates
                        {(Motor, 8.00) (AECS/CD2, 11.82)};
			\addplot + [fill=white,postaction={pattern=dots}] coordinates
                        {(Motor, 6.42)};
			\addplot + [fill=white,postaction={pattern=horizontal lines}] coordinates
                        {(Motor, 7.53)};
			\addplot + [fill=white,postaction={pattern=crosshatch}] coordinates
                        {(Motor, 7.89)};
			\legend{Delay1,Delay2,Delay3,Delay4,Delay5}
	\end{axis}
\end{tikzpicture}
