...
The previous results show that HRTCS and Printer were the only examples
that the compiler was able to find \textit{d} without relaxing upper
bound of \texttt{delay}. 
This is because there are two influential factors in
determining the relaxation: 1) execution speed of a target platform and 
%Figure~\ref{fig:graph} shows the accuracy
...
2) the nature of the program that runs on this platform. Clock frequency
of the processor directly impacts the value of WCRT. On the other hand,
programmer can write a program such that large data (or control)
computations in a single logical tick increase WCRT of the program.
Assuming we do not change the architecture of the execution platform
(i.e. JOP), the size of relaxation can only be reduced by modifying the
benchmark programs such that the critical paths are broken up using
\texttt{pause} constructs, thereby reducing the WCRT.
Figure~\ref{fig:improv} shows segments of code from AECS whose WCRT is
reduced by such technique.
...
identified this execution path caused lengthening the WCRT of the
program. In this case, we divide two consequtive statements, i.e.
...
technique is applied to Motor example to reduce WCRT of the program.
Note that we can also optimize algorithms of the data computations used
in each Java method to reduce WCRT. However it is out of scope of this
paper, hence not discussed here.

The result of the upper bound relaxation with reduced WCRT is shown in
Figure~\ref{fig:graph}. As one can see from the Figure, the upper bounds
for delay statements in AECS/CD2 has been decreased by 11.81\% which is
higher than all those in Motor which ranges from 7.54 to 8.05\%. It is
because AECS has relatively a large amount of data computation while
Motor is a completely control-dominated program. Therefore, the result
shows that the size of the relaxation depends on how designers write
their program as well as types of the application (i.e. data or control
dominated).
...
\begin{tikzpicture}[scale=0.9]
	\begin{axis}[
			...
			\addplot coordinates {(Motor, 8.05) (AECS/CD2, 11.82)};
			\addplot coordinates {(Motor, 8.00)};
			\addplot coordinates {(Motor, 6.42)};
			\addplot coordinates {(Motor, 7.53)};
			\addplot coordinates {(Motor, 7.89)};
